...
export const salesOrders = pgTable("sales_orders", {
  ...
  parentOrderId: uuid("parent_order_id").references(() => salesOrders.id),
...
export const supplierLpos = pgTable("supplier_lpos", {
  ...
  // Amendment tracking
  version: integer("version").default(1),
  parentLpoId: uuid("parent_lpo_id").references(() => supplierLpos.id),
  amendmentReason: text("amendment_reason"),
...
export const insertEnquiryItemSchema = createInsertSchema(enquiryItems).omit({ id: true, createdAt: true }).extend({
...
}).omit({ id: true, createdAt: true, updatedAt: true, parentQuotationId: true });
